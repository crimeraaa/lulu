 ...
 *  number of values will be returned by this particular function call.
 */
#define LULU_MULTRET (-1)


/** @brief A protocol that defines how memory is allocated within Lulu.
 *
 * @note(2025-08-02)
...
/** @brief The protocol that allows for C to interface with Lulu.
 *
 * @details
 ...
 *
 * @return
 *  The number of values pushed to the stack that will be used by the caller.
 */
typedef int (*lulu_CFunction)(lulu_VM *L);


/** @brief A generic interface to allow optimized reading of scripts/files.
 *
 * @details
 *  This is mainly to allow implementation of buffered reading for files
 *  received from the `fopen()` family.
 *
 * @param user_ptr
...
/** @brief(2025-06-11) Chapter 15.1.1 of Crafting Interpreters: "Executing
 *  instructions".
 */
typedef enum {
    LULU_OK,
...
/** @brief(2025-06-16) Chapter 18.1 of Crafting Interpreters: "Tagged
 *  Unions".
 */
typedef enum {
...
/** @brief Sets VM panic function, to be called when errors are thrown outside
 *  of protected calls.
 */
...
/** @brief Compiles the script read in by `reader` into a Lua function. */
LULU_API lulu_Error
lulu_load(lulu_VM *L, const char *source, lulu_Reader reader,
    void *reader_data);


/** @brief Calls a Lua or C function. Assumes stack has arguments you need.
 *
 * @details [POPPED: -(n_args + 1), PUSHED: +(n_rets), ERRORS: any]
 *
 * @note(2025-06-30) The function and its arguments are popped when done.
 *
 *  The return values, if any, are moved to where the function originally was.
 *  That is, the `-(n_args + 1)` stack slot, up to the `n_ret` slot above it,
 *  is overwritten.
 *
 *  If the function returned less than `n_rets` values then remaining slots are
 *  set to `nil`.
 */
LULU_API void
...
/** @brief Wraps `lulu_call()` in a protected call.
 *
 * @details
 *  [POPPED: -(n_args + 1), PUSHED: +(n_rets or 1 if caught), ERRORS: none]
 *
 * @return The error code, if any was thrown, or else `LULU_OK`.
 */
LULU_API lulu_Error
lulu_pcall(lulu_VM *L, int n_args, int n_rets);


/** @brief Wraps `function(vm, function_data)` in a protected call.
 *
 * @details [POPPED: -0, PUSHED: +(0 or 1 if caught), ERRORS: none]
 *
 *  At the start of the call, `function_data` is the 1st (and only) argument
 *  present on the stack. It can be retrieved by via a call to
 *  `lulu_to_userdata(vm, 1)`.
 *
 * @return
 *  The error code, if any was thrown, or else `LULU_OK`. Upon return, the
 *  stack is the same as it was before the call. If an error was thrown then
 *  the error message was pushed.
 */
LULU_API lulu_Error
lulu_cpcall(lulu_VM *L, lulu_CFunction function, void *function_data);


/** @brief Unconditionally throws a runtime error. Assumes an error message is
...
/** @return The type tag of the value at index `i`.
 *
 * @note(2025-07-20)
...
/** @return The nul-terminated type-name of the type tag `t`.
 *
 * @note(2025-07-20)
 *  C does not allow overloading of functions, and even if had, implicit
 *  conversions may lead to ambiguity if the compiler treats `lulu_Type` as
 *  being the same as `int`.
 *
 *  Thus to get the type name of the value at a particular index `i`, you would
 *  need to first call `lulu_type()` and pass it to here. E.g. `const char *s =
 *  lulu_type_name(vm, lulu_type(vm, -1));`;
 *
...
/**
 * @return
 *  1 if the value at index `i` is a `number` or a `string` which represents a
 *  valid number, or else 0.
 */
LULU_API int
...
/**
 * @return
 *  1 if the index `i` is a `string` or a `number` (which is always convertible
 *  to a string), else 0.
 */
LULU_API int
 ...
 *
 * @note(2025-07-20) No conversions in the stack occur.
 */
LULU_API int
...
/**
 * @return
 *  The `number` representation at the index `i` or else 0. `number` is read
 *  directly while `string` is checked then parsed. All other types return 0.
 *
 * @note(2025-07-20)
 *  Returning 0 is not enough to indicate the value was not of type `number`,
 *  because it is possible for the number 0 to be in the stack. You must call
 *  `lulu_is_number()` beforehand in that case.
 */
...
/**
 * @return
 *  The `number` representation of the value at index `i`, or else 0. The same
 *  conversion rules in `lulu_to_number()` apply.
 *
 * @note(2025-07-21)
 *  If the value at `i` is a number but cannot be accurately represented as a
 *  `lulu_Integer` (e.g. because it is a floating-point type), then the result
 *  is truncated in some unspecified way.
 */
LULU_API lulu_Integer
lulu_to_integer(lulu_VM *L, int i);


/** @brief Tries to converts value at index `i` to a `string`.
 *
 * @details
 *  Values of type `string` are read directly, while values of type `number`
 *  are replaced with their string representation. Other types are unchanged.
 *
 * @param [out] n
 *  Optional. Will stores the resulting string's length, if the value was
 *  indeed a string. Pass `NULL` to ignore.
 *
 * @return The nul-terminated string or else `NULL`.
 */
LULU_API const char *
 ...
 *  pointer. The only convertible types are either `userdata` or 'true' objects
 *  such as `table` or `function`.
 *
 *  `string` is not a 'true' object because it is immmutable and always passed
 *  by value conceptually.
 */
LULU_API void *
...
/** @brief Query the number of elements in the current stack frame. */
LULU_API int
lulu_get_top(lulu_VM *L);


/** @brief Set the end of current stack frame to the index `i`.
 *
 * @param i
 *  When 0, effectively pops all the elements as the stack frame will have a
 *  length of 0.
 *
 *  When `> 0`, only `i` values remain in the stack. E.g. `i == 1` will pop all
 *  elements until absolute index 1, leaving it to be the new top of the stack.
 *
 *  When `< 0`, only `lulu_get_top(vm) - (-i)` values remain in the stack. E.g.
 *  `i == -1` will pop all elements from the top of the stack, going down and
 *  including `i`. The value at `i` itself is also popped.
 *
 * @note(2025-07-20) `i` cannot be a pseudo index.
 */
LULU_API void
lulu_set_top(lulu_VM *L, int i);


/** @brief Replace stack index `i` with the top of the stack, popping it.
 *
 * @note(2025-07-20)
...
/** @brief Removes the value at the stack index `i`, and shift all elements
 *  that were above `i` down by one index.
 *
 * @note(2025-07-20)
 *  `i` cannot be a pseudo index, because it would not refer to an actual
 *  position in the stack.
...
/** @brief Pops `n` values from the top of the stack. */
LULU_API void
lulu_pop(lulu_VM *L, int n);


/** @brief Pushes a single `nil` value to the top of the stack. */
LULU_API void
lulu_push_nil(lulu_VM *L);


/** @brief Pushes `b` to the top of the stack as a `boolean` value. */
LULU_API void
lulu_push_boolean(lulu_VM *L, int b);


/** @brief Pushes `n` to the top of the stack as a `number` value. */
LULU_API void
lulu_push_number(lulu_VM *L, lulu_Number n);


/** @brief Pushes `i` to the top of the stack as a `number` value.
 *
 * @note(2025-07-21)
 *  If `i` cannot be represented accurately as a `lulu_Number`, it may be
 *  truncated (e.g. when `lulu_Number` is floating-point type).
 */
LULU_API void
lulu_push_integer(lulu_VM *L, lulu_Integer i);


/** @brief Pushes `p` as a `userdata` value. */
LULU_API void
lulu_push_userdata(lulu_VM *L, void *p);


/** @brief Pushes the string `s` with length `n`. */
LULU_API void
lulu_push_lstring(lulu_VM *L, const char *s, size_t n);


/** @brief Pushes the nul-terminated string `s`.  */
LULU_API void
lulu_push_string(lulu_VM *L, const char *s);


/** @brief Pushes a printf-style formatted string to the top of the stack.
 *
 * @param fmt
 *  The format string. Only specifiers in the regex `%[cdifsp]` are allowed. No
 *  precision, widths, or any modifiers are implemented.
 */
LULU_API const char *LULU_ATTR_PRINTF(2, 3)
lulu_push_fstring(lulu_VM *L, const char *fmt, ...);


/** @brief `va_list` wrapper similar to `lulu_push_fstring()`. */
LULU_API const char *
lulu_push_vfstring(lulu_VM *L, const char *fmt, va_list args);


/** @brief Pushes `cf` to the top of the stack as a `function` value,
 *  with `n_upvalues` from the top of the stack associated with it.
 */
LULU_API void
lulu_push_cclosure(lulu_VM *L, lulu_CFunction cf, int n_upvalues);

/** @brief Pushes `cf` as a `function` value, associating no upvalues. */
#define lulu_push_cfunction(vm, cf) lulu_push_cclosure(vm, cf, 0)


/** @brief Pushes a copy of relative/pseudo stack index `i`. */
LULU_API void
...
/** @brief Pops `key` from the stack and pushes `table[key]` in its place.
 *
 * @details [POPPED: -1, PUSHED: +1, ERRORS: any]
 *
 * @param table_index
 ...
 *
 *  1.) The key to be used is at index `-1`.
 *
...
/** @brief Pushes `table[key]` to the stack.
 *
 * @details [POPPED: -0, PUSHED: +1, ERRORS: ANY]
 *
 * @param table_index
 ...
 *
 * @return
 *  1 if `key` existed in the table, meaning `table[key]` was pushed. 0
 *  otherwise, meaning `nil` was pushed.
 *
 * @note(2025-07-20)
 *  It is possible for `table[key]` to have been mapped previously and to map
 *  to `nil`.
 */
LULU_API int
...
/**
 * @details [POPPED: -1, PUSHED: +1, ERRORS: any | memory]
 *
 * @param table_index
 *  The relative/psuedo stack index of the table we wish to set.
 *
 ...
 * @note(2025-07-20) Assumptions:
 *
 *  1.) The value to be used is at the index `-1`. When this
...
/**
 * @details [POPPED: -1, PUSHED: +(2 if continue else 0), ERRORS: any]
 *
 * @param table_index
 *  The relative/pseudo stack index of the table you wish to iterate over.
 *
...
/** @brief Pushes key `s` from the VM globals table.
 *
 * @return 0 if `s` does not exist in the globals table, else 1.
 */
#define lulu_get_global(vm, key) lulu_get_field(vm, LULU_GLOBALS_INDEX, key)


/** @brief Sets the global variable `s` to the current top of the stack.
 *  The value is then popped.
 */
#define lulu_set_global(vm, key) lulu_set_field(vm, LULU_GLOBALS_INDEX, key)


/** @brief Perform string concatenation of `n` elements from top of stack.
 *
 * @details [POPPED: -n, PUSHED: +1, ERRORS: any | memory]
 *
...
/** @brief Gets the length of the value stack index `i`.
 *
 * @return The length of the object, else 0.
 */
LULU_API size_t
...
/** @brief Manage/query the state of the garbage collector.
 *
 * @return
...
/** @return 1 if the value at index `i` is `nil`, else 0.
 *
 * @note(2025-07-19)
 *  For C functions called from Lua, since function arity (parameter count) is
 *  not known beforehand, arguments are not default-initalized to `nil` if not
 *  provided as the implementation has no way of knowing how many arguments you
 *  wanted.
 *
 *  Thus, to check for arguments that were not provided at all from a Lua
 *  function, use `lulu_is_none()`. C functions called from C (as in
 *  `lulu_pcall()`) do know how many arguments to expect because they were
 *  explicitly passed.
 */
#define lulu_is_nil(vm, i) (lulu_type(vm, i) == LULU_TYPE_NIL)


/** @return 1 if the value at index `i` is `none` else 0.
 *
 * @note(2025-07-19)
 *  Only indexes outside the current stack frame are type `none`. Only C
 *  functions can see this type because of their unsafe capability to request
 *  any stack index.
 */
#define lulu_is_none(vm, i) (lulu_type(vm, i) == LULU_TYPE_NONE)


/** @return 1 if stack index `i` is `none` or `nil`, else 0. */
#define lulu_is_none_or_nil(vm, i) (lulu_type(vm, i) <= LULU_TYPE_NIL)


/** @return 1 if stack index `i` is a `boolean`, else 0. */
#define lulu_is_boolean(vm, i) (lulu_type(vm, i) == LULU_TYPE_BOOLEAN)


/**
 * @return 1 if the value at index `i` is a `userdata`, else 0.
 *
 * @note(2025-07-20)
 *  `userdata` are generally safe to cast to pointers of some type `T`. A good
 *  example is the callback function of `lulu_cpcall()`, whose 1 and only
 *  argument is the userdata to be used.
 *
 *  Of course, the safety of casting to and from `void *` cannot be guaranteed
 *  by your compiler, much less `lulu`.
 */
#define lulu_is_userdata(vm, i) (lulu_type(vm, i) == LULU_TYPE_LIGHTUSERDATA)


/** @return 1 if stack index `i` is a `table`, else 0. */
#define lulu_is_table(vm, i) (lulu_type(vm, i) == LULU_TYPE_TABLE)


/** @return 1 if stack index `i` is a `function`, else 0. */
#define lulu_is_function(vm, i) (lulu_type(vm, i) == LULU_TYPE_FUNCTION)


/** @brief Ignore out parameter for length. All Lulu strings are C strings. */
#define lulu_to_string(vm, i) lulu_to_lstring(vm, i, NULL)


/** @brief Optimize pushing of string literals since size is known already. */
#define lulu_push_literal(vm, s) lulu_push_lstring(vm, s, sizeof(s) - 1)


/** @brief A fairly common operation is to bind a function to a global name. */
...
/** @brief An activation record.
 *
 * @link https://www.lua.org/pil/23.1.html
 *
 * @note(2025-07-23)
