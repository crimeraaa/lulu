...
{
    void *new_ptr = vm->allocator(vm->allocator_data, new_size, LULU_USER_ALIGNMENT, NULL, 0);
...
{
    void *new_ptr = vm->allocator(vm->allocator_data, new_size, LULU_USER_ALIGNMENT, old_ptr, old_size);
    return wrap_alloc(vm, new_ptr, new_size);
}
...
{
    vm->allocator(vm->allocator_data, 0, LULU_USER_ALIGNMENT, old_ptr, old_size);
}
